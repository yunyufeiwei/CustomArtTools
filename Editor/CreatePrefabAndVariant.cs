    ...
    public void CreatePrefabAndVariants(string OriginalprefabPath,string VariantPrefabPath , bool isNeedVariant)
    {
        ...
        if (selectedObjects.Length > 0)
        {
            ...
            {   
                ...
                if (!string.IsNullOrEmpty(selectObjectAssetPath) && Path.GetExtension(selectObjectAssetPath).ToLower()==".fbx")
                {
                    ...
                    PrefabUtility.ConvertToPrefabInstance(fbxInstance,selectedObjectFBX,convertToPrefabInstanceSettings,interactionMode);
                    
                    string OriginalPrefabName = emptyPrefabGo.name + ".prefab";  
                    string OriginalPrefabFullPath = Path.Combine(OriginalprefabPath, OriginalPrefabName); //全路径名，文件名+拓展名(类型) 
                    // 保存空的Prefab  
