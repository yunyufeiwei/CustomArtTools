...
public class TexturePropertySettings : EditorWindow
{
    ...
    {
        TexturePropertySettings window = (TexturePropertySettings)EditorWindow.GetWindow(typeof(TexturePropertySettings));
    ...
    private void SettingCommonProperty()
    {
        for (int i = 0; i < _textureImporters.Count; i++)
        {
            ...
            if (checkAlphaChannel == true)
            {
                ...
                if (_textureImporters[i].DoesSourceTextureHaveAlpha())   //DoesSourceTextureHaveAlpha(),源贴图是否有Alpha通道,返回值是一个布尔型
                {
