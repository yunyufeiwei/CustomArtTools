...
class ExportTerrain : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        {
            //如果没有地形，设置GUI绘制的信息模块
            GUILayout.Label("未找到地形");
            //绘制一个Button按钮，命名为取消，当场景中没有地形时显示，如果点击了Button按钮，则执行if内的方法
        ...
        //如果场景中有地形文件，则绘制两个GUI，一个为导出格式，一个为导出的精度
        saveFormat = (SaveFormat)EditorGUILayout.EnumPopup("导出格式", saveFormat);
        saveResolution = (SaveResolution)EditorGUILayout.EnumPopup("精度", saveResolution);
